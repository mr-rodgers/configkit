...
def version_name_matcher(path):
    ...
    shutil.copyfile(str(root / "config-0.1.json"),
                    str(tmp_path / "0.1" / "config.json"))
    shutil.copyfile(str(root / "credentials-0.1.json"),
                    str(tmp_path / "0.1" / "credentials.json"))
    shutil.copyfile(str(root / "config-1.0.json"),
                    str(tmp_path / "1.0" / "config.json"))
    shutil.copyfile(str(root / "credentials-1.0.json"),
                    str(tmp_path / "1.0" / "credentials.json"))
    shutil.copyfile(str(root / "config-0.2.json"),
                    str(tmp_path / "0.2" / "config.json"))
    return vnm
...
def name_version_matcher(path):
    ...
    shutil.copyfile(str(root / "config-0.1.json"),
                    str(tmp_path / "config" / "0.1.json"))
    shutil.copyfile(str(root / "credentials-0.1.json"),
                    str(tmp_path / "credentials" / "0.1.json"))
    shutil.copyfile(str(root / "config-1.0.json"),
                    str(tmp_path / "config" / "1.0.json"))
    shutil.copyfile(str(root / "credentials-1.0.json"),
                    str(tmp_path / "credentials" / "1.0.json"))
    shutil.copyfile(str(root / "config-0.2.json"),
                    str(tmp_path / "config" / "0.2.json"))
    return nvm  # lol


@pytest.fixture(params=['same_level_matcher', 'version_name_matcher', 'name_version_matcher'])
def matcher(request):
